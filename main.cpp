...
auto main() -> int
{
    ...
    auto generate_map = [&](auto chunk) -> map_type
    {
        ...
        for (fs::path const &pth : chunk)
        {
            ...
            {
                res[pth.extension().string()]->total_size = fd::file_size(pth);
            }
            else if (fs::is_directory)
            {
                ++directory_counter; // revisar si el contador atomico se incrementaba asi
            }
        }
        return res;
    };
    ...
    auto process_map = [&](map_type const &partial_map) -> void
    {
        ...
        {
            // Buscamos la extensión `ext` en el mapa principal `processed_data`.
            auto &existing_info = processed_data.try_emplace(ext, Extension_info{}).first->second;

            // Actualizamos el número de archivos y el tamaño total de la extensión `ext`.
            existing_info.num_files += info.num_files;
            existing_info.total_size += info.total_size;
        }
    };
    ...
    for (auto const &[ext, info] : processed_data)
    {
        ...
        pel::println("{}:       {}files:        {}bytes", ext, info.num_files, info.total_size);
    }
